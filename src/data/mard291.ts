 ...
 * MARD 291 色完整色盤。
 *
 * 資料來源：Pixel Beads「MARD 拼豆色號大全（2026 重新修訂版）」
 * 網站目前只提供色號與 HEX，沒有獨立顏色名稱，因此 name 暫以色號填入。
 */
export const mard291: MardColor[] = [
...
/**
 * 相容你目前專案既有的 mard221 import。
 * 不需修改 App.tsx；原本 import { mard221 } 會直接使用完整 291 色。
 */
export const mard221 = mard291

export const mardColorCount = mard291.length

export const mardColorById = new Map(
  mard291.map((color) => [color.id, color]),
)

export function findMardColor(id: string) {
  return mardColorById.get(id.trim().toUpperCase())
}

export function searchMardColors(query: string) {
  ...
  return mard291.filter((color) =>
    color.id.includes(normalized) ||
    color.name.toUpperCase().includes(normalized),
  )
}
...
export function validateMardPalette() {
  ...
  for (const color of mard291) {
    if (ids.has(color.id)) {
      errors.push(`重複色號：${color.id}`)
    }

    ids.add(color.id)

    if (!/^#[0-9A-F]{6}$/.test(color.hex)) {
      errors.push(`HEX 格式錯誤：${color.id} ${color.hex}`)
    }
  }

  if (mard291.length !== 291) {
    errors.push(`色號數量錯誤：預期 291，實際 ${mard291.length}`)
  }

  return {
    valid: errors.length === 0,
    count: mard291.length,
    errors,
  }
}
